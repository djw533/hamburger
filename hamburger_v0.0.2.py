...
def __gff2faa__(gff_file, fasta_file, strain,output_dir):
    ...
    out = open(output_dir+"/tmp_fasta.fa", "w")
    contigs = {}
    list_of_names = {}
    genes_and_contig = {}
    ...
    with open(output_dir+"/tmp_fasta.fa") as handle:
    ...
    out.close()
    os.remove(output_dir+"/tmp_fasta.fa")
...
def extract_a2b(start_gene, stop_gene, annotation, strain, cluster_num, upstream, downstream, contig, output_dir):
    ...
    for gff_line in annotation:
        ...
        id = gff_line.split()[8].split(";")[0].split("ID=")[1]
...
#8 Write out the stats
            ...
            with open(output_dir+"/operon_stats.csv", "a") as output:
            ...
            with open(output_dir+"/gggenes_input.csv","a") as output:
        ...
        with open(output_dir + "/strain_statistics.csv","a") as output:
